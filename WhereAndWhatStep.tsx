...
export default function WhereAndWhatStep({
  ...
  // âœ… Handle initial category from URL (when user clicks category card)
  ...
  // âœ… NEW: Advanced filtering and sorting
  ...
  // âœ… FIXED: Get current items based on single-leg or multi-leg
  ...
  const getCurrentItems = useCallback((): any[] => {
    if (isMultiLeg && segments.length > 0) {
      // âœ… CRITICAL FIX: For multi-leg, use items from first segment (outbound)
  ...
  // âœ… Get predefined packages
  const packages = useMemo(() => getItemPackages(), []);

  // âœ… ENHANCED: Filtered items with advanced sorting and filtering
  ...
  // âœ… FIXED: Update items in all segments (for multi-leg) or global items (for single-leg)
  ...
  const updateItemsInAllSegments = useCallback((updater: (items: any[]) => any[]) => {
    if (isMultiLeg && updateSegment && segments.length > 0) {
      // âœ… CRITICAL FIX: Update items in ALL segments to keep them synchronized
  ...
  const getItemQuantity = useCallback((itemId: any) => {
    // âœ… FIXED: For multi-leg, get quantity from first segment (all segments have same items)
    // This prevents double-counting and confusion
    if (isMultiLeg && segments.length > 0) {
      ...
      <VStack spacing={4}>
        {includeHeading && (
          <Heading size={{ base: "sm", md: "md" }} color="white">
            âœ… Selected Items ({currentItems.length})
          </Heading>
        )}
            ...
            <SimpleGrid columns={{ base: 1, md: 3 }} spacing={3} w="full">
              {pricingTiers.economy && (
                <Box p={3} bg="rgba(59, 130, 246, 0.2)" borderRadius="lg" border="1px solid" borderColor="rgba(59, 130, 246, 0.3)">
                  ...
                  <Text fontSize="lg" color="white" fontWeight="bold">
                    Â£{pricingTiers.economy.price?.toFixed(2) || '0.00'}
                  </Text>
                </Box>
              )}
              {pricingTiers.standard && (
                <Box p={3} bg="rgba(16, 185, 129, 0.2)" borderRadius="lg" border="1px solid" borderColor="rgba(16, 185, 129, 0.3)">
                  ...
                  <Text fontSize="lg" color="white" fontWeight="bold">
                    Â£{pricingTiers.standard.price?.toFixed(2) || '0.00'}
                  </Text>
                </Box>
              )}
              {pricingTiers.express && (
                <Box p={3} bg="rgba(239, 68, 68, 0.2)" borderRadius="lg" border="1px solid" borderColor="rgba(239, 68, 68, 0.3)">
                  ...
                  <Text fontSize="lg" color="white" fontWeight="bold">
                    Â£{pricingTiers.express.price?.toFixed(2) || '0.00'}
                  </Text>
                </Box>
              )}
            </SimpleGrid>
                        ...
                        <VStack align="stretch" spacing={2}>
                          <HStack justify="space-between">
                            ...
                            <Text color="white" fontSize="sm" fontWeight="700">
                              {segment.pricing?.total > 0 ? `Â£${segment.pricing.total.toFixed(2)}` : 'â€”'}
                            </Text>
                          </HStack>
                          <HStack spacing={2} color="whiteAlpha.900" fontSize="sm">
                            <Icon as={FaMapMarkerAlt} />
                            <Text>
                              {segment.pickupAddress?.postcode || 'Pickup'} â†’ {segment.dropoffAddress?.postcode || 'Drop-off'}
                            </Text>
                          </HStack>
                          {segment.datetime && (
            ...
            <VStack spacing={{ base: 4, md: 6 }} align="stretch">
              ...
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={{ base: 3, md: 4 }}>
                ...
                <FormControl isInvalid={step1.pickupDateChoice === 'known' && !!errors['step1.pickupDate']} isDisabled={step1.pickupDateChoice === 'unknown'}>
                  <FormLabel color="white" fontSize={{ base: "sm", md: "md" }}>ðŸ“… Select Date</FormLabel>
                  <Input
                    type="date"
                  ...
                  {step1.pickupDateChoice === 'unknown' && (
                    <Text color="gray.400" fontSize="xs" mt={2}>
                      You can confirm your date later â€” weâ€™ll hold todayâ€™s pricing.
                    </Text>
                  )}
                </FormControl>
                ...
                <FormControl isInvalid={!!errors['step1.pickupTime']} isDisabled={step1.pickupDateChoice === 'unknown'}>
                  <FormLabel color="white" fontSize={{ base: "sm", md: "md" }}>â° Select estimated time</FormLabel>
                  <Select
                    mb={4}
                  ...
                  >
                    <option value="08:00-12:00">8 AM - 12 PM ðŸŒ…</option>
                    <option value="12:00-16:00">12 PM - 4 PM â˜€ï¸</option>
                    <option value="16:00-18:00">4 PM - 6 PM ðŸŒ†</option>
                  ...
                  {step1.pickupDateChoice === 'unknown' && (
                    <Text color="gray.400" fontSize="xs" mt={2}>
                      Time optional â€” choose later without affecting your quote.
                    </Text>
                  )}
                </FormControl>
              </SimpleGrid>
                    ...
                    <CardBody p={4}>
                      <VStack spacing={4} align="stretch">
                        ...
                        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
                          {/* Subcategory Filter */}
                          <FormControl>
                            <FormLabel color="white" fontSize="sm" mb={2} fontWeight="600">ðŸ” Subcategory</FormLabel>
                            <Select
                              mb={3}
                          ...
                          {/* Sort By */}
                          <FormControl>
                            <FormLabel color="white" fontSize="sm" mb={2} fontWeight="600">ðŸ“Š Sort By</FormLabel>
                            <Select
                              mb={3}
                              value={sortBy}
                            ...
                            >
                              <option value="popular">â­ Popular First</option>
                              <option value="weight-asc">âš–ï¸ Lightest First</option>
                              <option value="weight-desc">ðŸ’ª Heaviest First</option>
                              <option value="name-asc">ðŸ”¤ Aâ†’Z</option>
                              <option value="name-desc">ðŸ”¤ Zâ†’A</option>
                            </Select>
                          </FormControl>
                        </SimpleGrid>
                        ...
                        {/* Weight Range Filter */}
                        <FormControl>
                          <FormLabel color="white" fontSize="sm" mb={2}>âš–ï¸ Weight Range (kg)</FormLabel>
                          <HStack spacing={3}>
                            <NumberInput
                              value={minWeight || ''}
            ...
            <VStack w="full" spacing={3}>
              {currentItems.length > 0 && (
                <Badge bg="green.600" color="white" p={3} borderRadius="lg" fontSize="md" w="full" textAlign="center">
                  âœ… {currentItems.length} Items Selected
                </Badge>
              )}
