...
async function runAllTests() {
  ...
  try {
    ...
    const reportPath = path.join(__dirname, '__tests__/temp', 'test-results.json');
    fs.writeFileSync(reportPath, JSON.stringify(reportData, null, 2));
    log(`\n💾 Test results exported to: ${reportPath}`, 'green');
...
function displayHelp() {
  ...
  log(`
Usage:
  node test-multiple-drops.js [options]
...
Examples:
  node test-multiple-drops.js                    # Run all tests
  node test-multiple-drops.js --test-type smoke  # Run smoke test only
  node test-multiple-drops.js --test-type load   # Run load test only
  node test-multiple-drops.js --help            # Show this help
